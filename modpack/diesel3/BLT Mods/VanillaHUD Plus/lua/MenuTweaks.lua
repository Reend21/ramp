if string.lower(RequiredScript) == "lib/managers/menumanager" then
	...
	function MenuCallbackHandler:show_empty_lobby()
		if not VHUDPlus:getSetting({"INVENTORY", "SHOW_EMPTY_LOBBY2"}, true) then
			return false
		end

		if menu_restore_beta then
			return false
		end
...
elseif string.lower(RequiredScript) == "lib/managers/crimenetmanager" then
		...
		if VHUDPlus:getSetting({"INVENTORY", "crnt_align"}, true) then
			...
			for i=1,xx do -- 224~1666 1442
				for j=1,yy do -- 165~945 780
					...
					local newX = 120+ 1642*i/xx
					local newY = 120+ 680*(i % 2 == 0 and j or j - 0.5)/yy
					if  (i >= 3) or ( j < 7 ) then
	...
	function CrimeNetGui:update_server_job(data, i, ...)
		...
		if colorizeCrNt and job and job.side_panel and job.side_panel:child('job_name') and not data.mutators and not data.is_crime_spree then
