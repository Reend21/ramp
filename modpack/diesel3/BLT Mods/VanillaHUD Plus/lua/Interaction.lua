if string.lower(RequiredScript) == "lib/units/beings/player/states/playerstandard" then
	...
	function PlayerStandard:_update_reload_timers(t, ...)
		...
		if PlayerStandard.SHOW_RELOAD and not PlayerStandard.HIDE_MELEE_RELOAD_PDTH and not PD3ReviveProgressDisplay then
	...
	function PlayerStandard:_interupt_action_reload(...)
		...
		if self._state_data.show_reload and PlayerStandard.SHOW_RELOAD and not PlayerStandard.HIDE_MELEE_RELOAD_PDTH and not PD3ReviveProgressDisplay then
			managers.hud:hide_interaction_bar(false)
			self._state_data.show_reload = false
		end
		return val
	...
	function PlayerStandard:_start_action_melee(t, input, instant, ...)
		...
		if not instant then
			...
			if PlayerStandard.SHOW_MELEE and self._state_data.meleeing and not hide_int_state[managers.player:current_state()] and not PlayerStandard.HIDE_MELEE_RELOAD_PDTH and not PD3ReviveProgressDisplay then
	...
	function PlayerStandard:_update_melee_timers(t, ...)
		...
		if PlayerStandard.SHOW_MELEE and self._state_data.meleeing and self._state_data.show_melee and not PlayerStandard.HIDE_MELEE_RELOAD_PDTH and not PD3ReviveProgressDisplay then
...
elseif string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:show_drill_interact(...)
		if VoidUI and VoidUI.options.enable_interact or PD3ReviveProgressDisplay then
			return
		end
...
elseif string.lower(RequiredScript) == "lib/managers/hud/hudinteraction" then
	...
	function HUDInteraction:set_interaction_bar_width(current, total)
		...
		if HUDInteraction.SHOW_TIME_REMAINING and not HUDInteraction.HIDE_TIMERS then
	...
	function HUDInteraction:show_interaction_bar(current, total)
		...
		HUDInteraction.HIDE_TIMERS = (NepgearsyHUDReborn and NepgearsyHUDReborn:HasInteractionEnabled())
		...
		if HUDInteraction.SHOW_TIME_REMAINING then-- and not HUDInteraction.HIDE_TIMERS then
			...
			self._interact_time:set_text(text)
			self._interact_time:set_outlines_visible(HUDInteraction.SHOW_TIME_REMAINING_OUTLINE and not HUDInteraction.HIDE_TIMERS)
